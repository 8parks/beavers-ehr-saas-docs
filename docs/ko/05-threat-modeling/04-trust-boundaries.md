...
JWT의 존재 여부만으로 요청을 신뢰할 수 없습니다. 토큰의 `scope`, tenant claim, role claim, tenant type claim, group claim이 서로 일관되는지 확인해야 합니다.

### `TB-03` Tenant 컨텍스트 경계

`tenant_registry`는 격리 정책의 기준 데이터입니다. schema 이름이나 S3 prefix를 사용자 입력으로 파생해서는 안 되며, 반드시 서버 측 allowlist에서 가져와야 합니다.

### `TB-04` 데이터베이스 경계

RDS Proxy 또는 커넥션 재사용이 존재하는 환경에서는 이전 세션의 tenant context가 남지 않도록 모든 트랜잭션에서 context를 재설정해야 합니다. 복구나 failover 후에도 동일한 검증이 필요합니다.

### `TB-06` 외부 제공 경계

연구기관용 데이터 제공은 병원 내부 진료 흐름과 별도 경계로 취급해야 합니다. 승인 상태, 제공 범위, 재식별 저감, 만료 시점이 모두 명시되어야 합니다.

### `TB-07` 운영 경계

운영자 비상 접근은 업무 경로보다 느슨해서는 안 됩니다. 정상 업무 사용자는 접근할 수 없는 데이터 평면을 예외적으로 여는 경로이므로, 더 엄격한 승인과 더 강한 추적성이 필요합니다.

### `TB-08` 외부 반출 경계

dataset 제공은 애플리케이션 내부 승인 상태와 실제 다운로드 행위가 분리되는 지점입니다. 따라서 presigned URL을 발급하는 시점뿐 아니라, 다운로드가 발생하는 시점에도 승인 상태와 만료 상태를 재검증할 수 있어야 합니다.

### `TB-09` 운영자 터널 경계

SSM 기반 포트 포워딩은 네트워크적으로는 안전한 선택이지만, 논리적으로는 운영자 단말에서 데이터 평면으로 직접 이어지는 경로입니다. 세션 승인, 세션 로그, 대상 인스턴스, 이후 DB 작업 증적이 하나의 체인으로 연결되지 않으면 운영자 행위의 사후 입증이 어렵습니다.

## 경계 검증 요구사항

- 모든 신뢰 경계는 해당 경계에서 검증되는 입력값과 금지되는 입력값이 정의되어야 합니다.
- 경계별로 성공 로그와 실패 로그 모두를 남겨야 합니다.
- 경계별 음성 테스트는 배포 파이프라인 또는 정기 검증 절차에 포함되어야 합니다.
