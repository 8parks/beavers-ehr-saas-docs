...
- `TB-04`, `TB-05`, `TB-07`은 PHI 접근 또는 운영자 예외 경로와 직접 연결되므로 가장 먼저 검토합니다.
- `TB-02`, `TB-03`, `TB-08`은 tenant 컨텍스트와 데이터 전달 경로의 일관성에 영향을 주므로 설계 변경 시 우선 검토합니다.
- `TB-01`, `TB-06`, `TB-09`는 외부 요청, 외부 제공, 운영자 단말 경계에 해당하므로 배포 전 점검과 정기 검토에 포함합니다.
