...
- key admin과 key user 역할이 분리되지 않음.
- clinical file key, dataset key, signing key가 동일하거나 policy 범위가 과도함.
- `kms:ViaService`, encryption context, resource tag 조건이 없음.
- key usage에 대한 기대 principal 목록과 경보 기준이 정의되지 않음.

## 위협 시나리오

1. 운영자 또는 Lambda 역할 하나가 여러 KMS 키에 대해 `Decrypt`, `Sign`, `GenerateDataKey` 권한을 동시에 보유한 상태가 형성됨.
2. 해당 역할을 획득한 사용자가 원본 임상 파일 키로 객체를 복호화하거나, 기록 서명 키로 수정된 payload에 새로운 서명을 생성함.
3. key usage 자체는 로그로 남더라도 기대 principal 목록과 정상 패턴이 정의되어 있지 않으면 이상 사용이 정상 운영과 구분되지 않음.
4. 결과적으로 원본 데이터 복호화, 가짜 서명 생성, 데이터셋 경로 권한의 과도 확장이 동시에 성립할 수 있음.
