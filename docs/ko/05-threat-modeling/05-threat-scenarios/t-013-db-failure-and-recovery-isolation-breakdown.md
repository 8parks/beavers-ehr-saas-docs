...
- DR runbook이 가용성 복구만 다루고 tenant isolation 검증을 포함하지 않음.
- snapshot restore 후 schema, RLS, parameter drift 검증이 없음.
- 장애 대응 중 임시 권한이 과도하게 부여됨.
- failover 시 writer/reader 전환과 애플리케이션 재연결 검증이 없음.

## 위협 시나리오

1. Aurora writer 장애, failover, snapshot restore, 리전 수준 장애 복구 이벤트가 발생함.
2. 운영자는 서비스 재개를 우선해 우회 경로나 수동 복구 절차를 사용하고, 애플리케이션은 재연결을 시도함.
3. 복구된 DB에서 RLS, schema binding, parameter 설정, 감사 로깅 중 일부가 누락되면 정상 트래픽이 보안 결함 상태로 재개됨.
4. 사용자는 복구가 끝난 것으로 인식하지만, 실제로는 cross-tenant 차단 실패나 감사 공백이 남아 있는 상태가 지속될 수 있음.
...
- DR drill에는 가용성 검증 외에 cross-tenant 차단 테스트가 포함되어야 합니다.
...
| drill 결과 | failover 시간, restore 성공 여부, cross-tenant 차단 테스트 결과 | 반기별 |
