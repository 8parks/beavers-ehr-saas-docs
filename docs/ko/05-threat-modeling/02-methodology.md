...
본 섹션은 단일 프레임워크를 기계적으로 적용하지 않습니다. 기본 절차는 OWASP 위협 모델링 프로세스를 따르되, 운영 유지관리 원칙은 AWS Well-Architected를 따릅니다. 분류 체계는 STRIDE와 LINDDUN을 병행합니다. 그 이유는 본 가이드라인이 일반적인 웹 애플리케이션 위협뿐 아니라, 가명정보 제공과 재식별 가능성까지 동시에 다루기 때문입니다.
...
5. 검증 항목과 배포 차단 기준을 문서화합니다.
6. 운영 중 발생한 변경 사항과 사고 학습 결과를 반영해 갱신합니다.

## 위협 진술 작성 규칙

본 섹션에서 개별 위협을 문서화할 때는 다음 규칙을 적용합니다.

- 위협은 반드시 특정 자산과 특정 신뢰 경계에 매핑되어야 합니다.
- 위협은 공격자 관점의 남용 경로 또는 운영 실패 관점의 실패 모드로 서술합니다.
- 위협은 “무엇이 잘못될 수 있는가”에 그치지 않고 “어떤 통제가 실패하면 성립하는가”를 포함해야 합니다.
- 위협은 구현 세부를 과도하게 일반화하지 않습니다. 예를 들어, “인가 우회” 대신 `tenant_registry` 오염, `set_config('app.tenant_id', ..., true)` 누락, `APPROVED` 상태 우회와 같이 본 구조에서 실제 성립하는 결함 형태를 명시합니다.
...
| Assess your work | 위협 우선순위, 배포 차단 기준, 검증 항목, 잔여 위험 명시 |
...
연구 데이터셋과 감사 메타데이터는 STRIDE만으로 충분히 설명되지 않습니다. 본 가이드라인은 가명정보 제공과 재식별 가능성을 다루므로, 데이터 제공 범위와 반복 추출에 따른 Privacy 위험을 별도로 평가해야 합니다.
...
본 섹션은 정량 점수 대신 `높음`, `중간`, `낮음`의 정성 등급을 사용합니다. 이는 현재 가이드라인 단계에서 합리적인 비교와 배포 판단을 지원하기에 충분하기 때문입니다. 다만 위협 시나리오 카탈로그에는 설계 또는 배포 판단에 영향을 주지 않는 경미한 위협을 수록하지 않으므로, 전체 분포는 고위험과 중위험에 집중될 수 있습니다. 등급은 다음 요소를 함께 고려해 결정합니다.
...
| 위험도 | 해석 |
|--------|------|
| 높음 | 배포 차단 또는 즉시 수정 대상임. 우회 경로나 보완 통제만으로 수용하지 않음 |
| 중간 | 보완 통제와 검증 결과에 따라 배포 가능 여부 결정. 운영 경보와 후속 개선 일정 필요 |
| 낮음 | 직접 사고 가능성은 낮으나 구조적 취약점의 전조일 수 있으므로 정기 점검 항목으로 유지 |

## 방법론 적용의 산출물

본 방법론의 결과물은 위협 목록에 그치지 않습니다. 최소한 다음 산출물이 남아야 합니다.
...
- 문서에서 수용한 예외는 운영 runbook과 사고 대응 절차에서 같은 용어로 추적 가능해야 합니다.
