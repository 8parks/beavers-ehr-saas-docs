...
> 본 섹션은 멀티테넌트 EHR SaaS 아키텍처의 배포 적합성을 판단하기 위한 위협 모델과 통제 기준을 정의합니다.

## 목적

본 섹션은 설계 검토, 구현 검토, 배포 승인, 운영 변경 심사에 공통 적용하는 통제 기준서입니다. 따라서 본 섹션에 수록되는 위협은 자산, 신뢰 경계, 권한 모델, 운영 절차를 기준으로 실제 워크로드에서 성립 가능하거나 명시적으로 배제되어야 하는 실패 모드로 한정합니다.

본 섹션이 답하려는 질문은 다음과 같습니다.
...
이 범위에는 애플리케이션 데이터 평면뿐 아니라 관리 평면과 예외 경로가 포함됩니다. 구체적으로는 Cognito 기반 인증, API Gateway 진입점, Lambda 인가 로직, `tenant_registry`, RDS Proxy, Aurora, S3, DynamoDB, KMS, CloudTrail, SSM 운영 경로를 모두 분석 대상으로 봅니다.

## 작성 기준

본 섹션은 다음 공식 레퍼런스를 기준으로 구성했습니다.
...
위협 모델은 일회성 산출물이 아니라 유지되는 등록부로 운영합니다. 다음 변경은 모두 위협 모델 재검토 사유입니다.
...
또한 다음 중 하나라도 발생하면 해당 위협 시나리오를 재평가해야 합니다.
