...
본 절은 개별 위협 시나리오에 대해 어떤 통제가 반드시 준비되어야 하는지 정리한다. 배포용 가이드라인이므로 “권장 사항”보다 “배포 차단 조건”을 우선한다.
...
통제는 “존재한다”는 주장만으로 충분하지 않다. 설계 검토 또는 배포 승인 시 최소한 다음 유형의 증빙이 확인되어야 한다.
...
- Lambda는 Cognito authorizer 결과만 신뢰하지 않고 `tenant_id`, `tenant_type`, `role`, `group` 일관성을 다시 확인해야 한다.
- 환자 조회는 `patient_id + tenant_id`만으로 충분하지 않으며, 실제 운영 배포에서는 patient-level 정책이 추가되어야 한다.
- 성공 로그와 거부 로그 모두를 PHI 감사 이벤트로 남겨야 한다.

### `S2. 환자 진료 기록 작성 및 조회`

- 기록 생성은 인가된 의료진만 수행할 수 있어야 하며, 병원 관리자는 기본적으로 차트 생성 권한을 갖지 않아야 한다.
- record 본문과 attachment metadata는 동일한 무결성 모델로 보호되어야 한다.
- presigned URL 기반 업로드는 파일 무결성과 tenant-bound 경로 통제를 함께 가져야 한다.

### `S3. 연구기관용 데이터셋 생성 및 제공`

- 연구자는 원본 DB 또는 clinical file bucket에 직접 접근해서는 안 된다.
- dataset 생성은 승인 후 내부 파이프라인에서만 실행되어야 하며, 승인 전 객체가 생성되어서는 안 된다.
- 직접 식별자 제거 외에 generalization, suppression, 반복 추출 위험 검토가 필요하다.
- `APPROVED`에서 `READY`로 전환되는 구간은 단순 배치 처리 단계가 아니라 통제 단계로 취급해야 한다.

### `S4. Tenant 온보딩`

- 온보딩은 보안 경계 복제 작업이므로 변경 승인, uniqueness 검증, 상태 이력 보존이 필요하다.
- `tenant_registry` 값은 향후 오프보딩, 감사, 사고 조사 기준이 되므로 별도 보호 대상이다.

### `S5. Tenant 오프보딩`

- 계정 비활성화와 데이터 파기를 동일 단계로 처리해서는 안 된다.
- DB schema, S3 객체, generated dataset, presigned URL, cache, backup 식별 정보까지 범위에 포함해야 한다.
- 파기 증적과 예외 승인 절차를 함께 남겨야 한다.

### `S6. 운영자 비상 접근`

- break-glass는 일반 운영 절차를 대체하는 수단이 되어서는 안 된다.
- 세션 시작부터 종료까지의 모든 행위는 승인 기록과 함께 연결 가능해야 한다.

### `S7. 데이터베이스 장애 및 복구`

- 복구 목표는 서비스 재개가 아니라 보안 속성을 유지한 상태의 재개다.
- DR drill은 가용성 검증과 함께 cross-tenant 음성 테스트를 포함해야 한다.

## 프로덕션 배포 차단 기준

다음 항목 중 하나라도 충족하면 이 아키텍처는 프로덕션 배포 대상이 아니다.

- tenant A 토큰으로 tenant B 데이터가 한 건이라도 조회된다.
- 병원 관리자 기본 권한으로 상세 진료 기록 조회 또는 작성이 가능하다.
- 연구자가 원본 PHI 저장소 또는 `dataset.generate` 내부 경로를 직접 호출할 수 있다.
- 승인되지 않은 dataset object가 생성되거나 다운로드될 수 있다.
- 가명처리 기준이 정의되지 않았거나 placeholder 처리만으로 dataset이 생성된다.
- SaaS 운영자 또는 일반 운영 role에 PHI 관련 KMS `Decrypt` 권한이 있다.
- `tenant_registry` 변경이 승인과 감사 없이 수행된다.
- 오프보딩된 tenant의 세션, presigned URL, dataset artifact가 남아 있다.
- PHI 조회, 수정, 다운로드 이벤트가 애플리케이션 감사 로그에 남지 않는다.
- CloudTrail 또는 app audit가 중단되어 사고 후 증적 확보가 불가능하다.
- break-glass 접근이 승인, 세션 로깅, 자격증명 만료 없이 상시 가능하다.

위 항목은 개별 결함 목록이 아니라, 보안 속성의 붕괴를 뜻하는 징후다. 하나라도 충족하면 부분 보완 후 추후 개선 대상으로 남길 수 없으며, 재검증 전까지 배포를 보류해야 한다.

## 잔여 위험

모든 통제를 적용해도 다음 잔여 위험은 남는다.

- 정상 계정과 정상 단말을 가진 내부자의 업무상 오남용을 완전히 제거할 수는 없다.
- 연구 데이터셋은 외부 데이터와 결합될 때 재식별 위험이 다시 커질 수 있다.
- 병원별 운영 성숙도 차이로 인해 동일 아키텍처라도 실제 보안 수준이 달라질 수 있다.
- 대규모 장애 상황에서는 예외 경로 사용이 증가해 평시보다 높은 운영 위험이 일시적으로 발생할 수 있다.

잔여 위험은 문서화, 정기 점검, 모의훈련, 침투 테스트, DR drill, 감사 로그 검토를 통해 지속적으로 관리해야 한다.
