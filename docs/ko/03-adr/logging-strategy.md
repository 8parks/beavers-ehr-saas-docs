...
HIPAA Security Rule 164.312(b)는 PHI를 포함하는 시스템의 활동을 기록하고 검토할 수 있는 감사 통제를 Required 요구사항으로 명시합니다. 개인정보보호법 제29조 제5항은 접속 기록의 위변조 방지 조치를 안전조치 의무로 규정합니다. 로그가 변조될 수 있다면 감사 통제는 의미를 잃습니다.
...
HIPAA 164.312(b)가 요구하는 감사 기록은 "누가 어떤 AWS 리소스를 호출했는지"만으로는 부족합니다. "누가 어떤 환자의 어떤 데이터를 조회했는지"까지 확인할 수 있어야 HIPAA가 의도한 감사 추적이 가능합니다. CloudTrail은 전자를, CloudWatch Logs는 후자를 담당합니다. 두 레이어를 분리한 이유는 각각 다른 수준의 규제 요구사항을 충족하기 위해서입니다.
