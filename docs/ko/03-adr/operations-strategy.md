...
멀티테넌트 SaaS에서 모든 API 요청은 해당 테넌트가 현재 어떤 상태인지를 먼저 확인해야 한다. 테넌트 상태를 코드나 설정 파일이 아닌 런타임에 조회 가능한 중앙 레지스트리에서 관리하면, 온보딩·정지·오프보딩 상태 변화를 즉시 반영할 수 있다.

### 결정

**DynamoDB `ehr-tenant-registry` 테이블을 Tenant 상태의 단일 진실 공급원으로 사용하며, 모든 Lambda는 요청 처리 전 이 테이블에서 `status`를 확인한다.**
...
온보딩 시점에 생성되는 Aurora schema, RLS policy, S3 prefix, Cognito custom attribute, tenant registry 항목이 서로 일치하지 않으면 이후 전체 데이터 격리가 깨진다. 구성 요소를 표준화된 절차 안에서 함께 생성하고, 검증 통과 후에만 `ACTIVE`로 전환한다.

### 결정

**IaC 기반 프로비저닝 파이프라인을 통해 모든 구성 요소를 원자적으로 생성하고, cross-tenant 차단 테스트 통과 후에만 `ACTIVE`로 전환한다.**
...
> 검증 항목, 역할 분리, 증빙 요건 등 구체적인 운영 절차는 [운영 가이드 — Tenant 온보딩](../06-runbook/01-tenant-lifecycle#tenant-onboarding)을 참고한다.
...
서비스 해지 시 즉시 물리 삭제가 아니라 단계적 처리를 원칙으로 한다. 의료법상 보존 의무 기간이 남은 데이터는 `ARCHIVED` 상태로 전환하여 접근만 차단하고 즉시 파기하지 않는다. 오프보딩 과정이 다른 테넌트의 서비스에 영향을 주어서는 안 된다.

### 결정

**레지스트리 기반 즉시 접근 차단 → 데이터 export 유예 기간 부여 → 보존 기간 확인 → 단계적 파기 순으로 처리한다. 파기 이력은 파기 대상에서 제외하고 별도 보존한다.**
...
> 역할 분리, 잔존 자산 등록부, 증빙 요건 등 구체적인 운영 절차는 [운영 가이드 — Tenant 오프보딩](../06-runbook/01-tenant-lifecycle#tenant-offboarding)을 참고한다.
...
SaaS 운영자가 테넌트 내부 의료 데이터에 직접 접근하지 않는 것이 기본 원칙이다. 이를 테넌트별 KMS CMK 기반 암호화와 CloudTrail 로그로 증빙한다. 단, 응급 진료 지원·장애 대응·보안 사고 조사처럼 환자 안전 또는 서비스 연속성에 영향을 미치는 상황에서는 Break-glass 절차를 통해 제한적 접근을 허용한다.

HIPAA Security Rule은 긴급 접근 절차 수립을 요구하며, 모든 접근은 기록·검토되어야 한다.

### 결정

**MFA 재인증 + AWS STS 임시 자격증명(TTL) + Break-glass 전용 IAM Role을 조합하고, 접근 로그를 일반 감사 채널과 분리된 전용 Log Group에 기록한다. 접근 발생 즉시 병원 관리자에게 알림을 발송하고 사후 보고서 작성을 의무화한다.**
...
> 중요도 분류, 단계별 대응 절차, 복구 선언 기준 등 구체적인 사고 대응 절차는 [운영 가이드 — 사고 대응](../06-runbook/03-incident-response)을 참고한다.
