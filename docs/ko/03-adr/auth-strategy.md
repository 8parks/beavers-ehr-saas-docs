...
RBAC가 부족한 경우는 테넌트 간 격리에서만 생기지 않습니다. **같은 병원 내에서도 역할만으로 접근을 제어할 수 없는 상황이 있습니다.** 대표적인 예가 정보보호 환자입니다. 정보보호 환자의 진료 기록은 같은 병원 소속 의사라도 사전에 지정된 담당 의료진만 접근할 수 있어야 합니다. 이 경우 `doctor` 역할 자체는 통과하지만, 해당 환자에 대한 접근 권한이 있는지를 추가로 확인해야 합니다.

이를 위해 Cognito Custom Attribute를 활용합니다. 담당 의료진 지정 여부를 속성으로 관리하고, 서비스 레이어에서 역할과 함께 이 속성을 검증해 접근을 제어합니다.
