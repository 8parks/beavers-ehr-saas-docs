...
| 통제 항목 | 수단 | 규제 |
|---|---|---|
| Tenant 레지스트리 관리 | DynamoDB `ehr-tenant-registry`(`tenant_id`, `schema_name`, `status`: PROVISIONING→ACTIVE, S3 Prefix 등록) | [접근통제](../04-compliance#접근통제) |
| 스키마 자동 프로비저닝 | IaC/Terraform 기반 Aurora schema, table, index, RLS policy, grant 생성 | [접근통제](../04-compliance#접근통제) |
| Registry 기반 접근 통제 | `status == ACTIVE` 확인, `schema_name` allowlist 확인, `app.tenant_id` transaction-local 설정 | [접근통제](../04-compliance#접근통제) |
| 인증·계정 관리 | Cognito User Pool 그룹 + custom attribute 설정, `FORCE_CHANGE_PASSWORD`, MFA 적용, 초기 계정 3개 제한 | [인증](../04-compliance#인증) |
| 자격증명 관리 | Secrets Manager(초기 자격증명 + DB 접속 정보 저장) | [암호화](../04-compliance#암호화) |
| 검증 | cross-tenant 차단 테스트 수행 후 `ACTIVE` 전환 | [접근통제](../04-compliance#접근통제) |
| 감사 로그 | CloudTrail + CloudWatch Logs(온보딩 이벤트 전 과정 기록) | [감사 로그](../04-compliance#감사-로그) |
