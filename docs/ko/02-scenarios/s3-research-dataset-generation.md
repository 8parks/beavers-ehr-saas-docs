...
| 통제 항목 | 수단 | 규제 |
|---|---|---|
| 인증 | Amazon Cognito User Pool + JWT | [인증](../04-compliance#인증) |
| 승인 기반 접근 통제 | DynamoDB `dataset-request-status`(상태 관리 + TTL 자동 만료), `approve-dataset` Lambda(병원 관리자 명시적 승인) | [접근통제](../04-compliance#접근통제) |
| 비식별 처리 파이프라인 | `generate-dataset` Lambda(직접 식별자 제거·가명 ID 치환·준식별자 일반화·재식별 위험 기준 확인) | [가명정보에 대한 정의 및 보호 정리](../04-compliance#가명정보에-대한-정의-및-보호-정리) |
| 테넌트 격리 | Aurora PostgreSQL Schema-per-Tenant + RLS, 데이터셋 버킷 Dataset A/B 분리 저장 | [접근통제](../04-compliance#접근통제) |
| DB 접근 통제 | RDS Proxy(직접 접근 차단), Private Subnet + NACL + Security Group | [접근통제](../04-compliance#접근통제) |
| 파일 저장 | 진료파일 버킷(Tenant별 Prefix), 데이터셋 버킷(Dataset A/B), S3 Gateway Endpoint(인터넷 미경유), KMS CMK 암호화 | [암호화](../04-compliance#암호화) · [데이터 저장](../04-compliance#데이터-저장) |
| 자격증명 관리 | Secrets Manager(DB 접속 정보 런타임 조회), KMS CMK(주기적 교체), KMS Interface Endpoint | [암호화](../04-compliance#암호화) |
| 감사 로그 | CloudTrail(AWS API 호출 기록), CloudWatch Logs(요청·승인·생성·다운로드 이벤트), 로그 S3 Tenant별 Prefix 분리 저장 | [감사 로그](../04-compliance#감사-로그) |
