...
| 권한부여 | RBAC: Cognito 그룹 doctor/hospital_admin | [접근통제](../04-compliance#접근통제) |
| 테넌트 격리 | JWT Claim `custom:tenant_id` + Aurora PostgreSQL Schema-per-Tenant + RLS | [접근통제](../04-compliance#접근통제) |
| 로깅 | Cross-Tenant 접근 시도 로그(CloudWatch Metrics + CloudWatch Alarm), CloudTrail(AWS API 호출 기록) | [감사 로그](../04-compliance#감사-로그) |
| 이상 행위 탐지 | GuardDuty(이상 인증 패턴 탐지) | [탐지](../04-compliance#탐지) |
